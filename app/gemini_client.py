...
_DRAFT_SCHEMA = {
    ...
    "items": {
        ...
        "properties": {
            ...
            "workstream_id": {"type": "STRING", "nullable": True},
...
def extract_tasks(prompt: str, workstreams: list[dict], profiles: list[dict]) -> list[dict]:
    """Calls Gemini to turn a free-text prompt into a list of draft tasks.

    workstreams/profiles are the full candidate lists the model is allowed to
    pick ids from - the caller is responsible for re-validating any returned
    workstream_id/assignee_id against these before trusting them, since a
    ...
    workstream_lines = "\n".join(
        f"- {w['id']} | {w['project_name']} / {w['name']}" for w in workstreams
    )
...
- workstream_id: pick the single best-matching id from this list based on any project/client/topic
